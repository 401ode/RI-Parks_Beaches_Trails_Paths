...
def replace_boolean_values(series=pd.Series):
    """
    A convenience DRY method to accommodate the needs of
...
def replace_binary_values(series=pd.Series):
    """
    A convenience DRY method to accommodate the needs
    of Google Sheets/JavaScript.
    """
...
def filter_inappropriate_sites(df):
    ...
    df = ~df[df['OwnerTyp'].str.contains('STA') & df['GoRI_TestFlag'] == nan & df['GoRI_TestFlag'] == nan]
...
def calc_running(df):
    ...
    May need to create a running path column beforehand that selects out
    the walking paths and trails suitable for running. Currently, Cliff
    Walk (a walking path flag) is but should not be a running path.
...
def calc_biking(df):
    ...
    GitHub Issue #11 per @theryankelly:
        Current Flag for Running (calc_running method above) Includes 1
        or > from field: "Bike Path"

    Interesting problem because it's yes if either:
        1) a calculated field/column contains 'yes'
        2) a base column contains 1.

    Easiest answer is to convert/replace 1/0 for 'yes'/'no'
    for Bike Path. Added DRY method up top.
...
def filter_all_n(df):
    """
    GitHub Issue #15
    Using calculated fields of Running, Swimming, Biking, 
    Paddling, Hiking, Walking and "Type" State Park. Filter list of assets.
